...
/* See app/c/consult/page.tsx — same optional-offices shape, same reason. */
const OFFICES = Object.entries(
  ((CONSULT as unknown as { offices?: Record<string, {
    officeShort: string; address: string; city: string
...
const OFFICE_ADDR = Object.fromEntries(
  OFFICES.map((o) => [o.key, { address: o.address, city: o.city, officeShort: o.officeShort }])
)
    ...
    <div id="consult">
      ...
      <main className="thanks">
        <section className="thanks-head">
          ...
          {MULTI ? (
            <BookingSummary offices={OFFICE_ADDR} />
          ) : (
            <BookingSummary address={SINGLE.address} city={SINGLE.city} />
          )}
        </section>
        ...
        <div className="callnote">
          ...
          <a href={`tel:${MULTI ? OFFICES[0].phoneRaw : CONSULT.phoneRaw}`}>
            ...
            {MULTI ? OFFICES.map((o) => `${o.officeShort} ${o.phoneDisplay}`).join('  ·  ') : CONSULT.phoneDisplay}
          </a>
        </div>
      </main>
      ...
      <footer className="consult-footer">
        ...
        <p className="cf-line">{MULTI ? OFFICES.map((o, i) => (
          <span key={o.key}>
            {i > 0 && <> &nbsp;·&nbsp; </>}
