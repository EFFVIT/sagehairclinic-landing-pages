...
import {
  ...
  resolveOffice, isFormat, MULTI_OFFICE, type ConsultFormat,
} from '@/lib/consult'
...
export async function POST(req: NextRequest) {
  let lead: Record<string, string | undefined>
  try {
    lead = await req.json()
  } catch {
  ...
  /* Office first: every later call — contact, custom fields, appointment —
     goes to the sub-account this resolves to. A missing or unknown office is a
     400, never a fallback, because booking a patient into the wrong practice
     succeeds silently and looks right on every screen. */
  let office
  try {
    office = resolveOffice(lead.office)
  } catch (err) {
  ...
  if (!isFormat(office, format)) return NextResponse.json({ error: 'unknown format for this office' }, { status: 400 })
  ...
  try {
    const contactId = await upsertContact({ ...lead, phone: `+1${phone}` }, officeArg)

    /* Attribution write is reported, never assumed. writeCustomFields returns a
       reason when that OFFICE's map is empty so a silent no-op cannot read as
       success — and the map is per sub-account, because custom field keys do
    ...
    const appointment = (await createAppointment({
      contactId, format, startIso: String(lead.slot), office: officeArg,
    })) as unknown as { id?: string; appointment?: { id?: string } }
    ...
    console.error('[consult/book]', office.key, e?.status, e?.message, e?.body)
    ...
    if (e?.status === 409 || /slot/i.test(String(e?.body?.message || ''))) {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: 'could not complete the booking', detail: e?.status || null },
      { status: 502 }
    )
  }
}
