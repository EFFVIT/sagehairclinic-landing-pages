...
/* ============================================================================
   ...
   OFFICE STEP, added 2026-09-25 and driven entirely by config, which is what
   the previous version of this comment asked for. A config that declares
   `offices` gets a real first step where the patient picks one; a config that
   does not is byte-for-byte unchanged and never renders it. The chosen office
   then supplies the address, the phone number and the calendars for every
   later step, because on a two-sub-account client those are properties of the
   OFFICE and not of the practice.
...
/* Offices are optional. `OFFICES` is [] for the nine single-office repos, which
   is what makes the office step disappear for them rather than render empty. */
type OfficeEntry = {
  key: string; label: string; officeShort: string; inPersonWhere: string
  address: string; city: string; phoneDisplay: string
}
const OFFICES: OfficeEntry[] = Object.entries(
  ((CONSULT as unknown as { offices?: Record<string, Omit<OfficeEntry, 'key'>> }).offices) || {}
).map(([key, o]) => ({ key, ...o }))
...
export default function ConsultFunnel() {
  ...
  /* The office the patient picked, or null before they have. Every
     office-scoped string below reads through this rather than off CONSULT, so
     a two-office client can never show the Moorestown address to somebody
     booking Metuchen — which would be wrong and would look completely fine. */
  const office = useMemo(
    () => (MULTI ? OFFICES.find((o) => o.key === v.office) || null : null),
    [v.office]
  )
  const whereLine = office ? office.inPersonWhere : (SINGLE.inPersonWhere ?? '')
  const shortName = office ? office.officeShort : (SINGLE.officeShort ?? '')
  const callNumber = office ? office.phoneDisplay : CONSULT.phoneDisplay

  const ready = useMemo(() => {
    const id = STEPS[step].id
    if (id === 'office') return !!v.office
    if (id === 'contact') {
  ...
  const loadSlots = useCallback(async (format: string, officeKey?: string) => {
    setSlots(null); setSlotsError('')
    try {
      /* The office is part of the QUESTION, not a detail of it: the same
         format at the other practice is a different calendar in a different
         sub-account. Omitting it on a multi-office client is a 400 by design. */
      const q = new URLSearchParams({ format })
      if (MULTI && officeKey) q.set('office', officeKey)
      const r = await fetch(`/api/consult/slots?${q.toString()}`)
      if (!r.ok) throw new Error(String(r.status))
      const d = await r.json()
      setSlots(d.slots || [])
    } catch {
      /* An empty list and a failed read are different claims. */
      setSlotsError(`We could not load times just now. Call ${callNumber} and we will book it for you.`)
    }
  }, [callNumber])
  ...
  const advance = async () => {
    ...
    const next = step + 1
    setStep(next)
    if (STEPS[next].id === 'time') loadSlots(v.format, v.office)
    ...
    try {
      const r = await fetch('/api/consult/verify', {
        ...
        body: JSON.stringify({
          ...
          format: v.format, office: v.office,
        }),
      })
      const d = await r.json()
      ...
      setVErr(`We could not send the code. Check the number, or call ${callNumber} and we will book it for you.`)
      ...
      if (r.ok && d.booked) {
        ...
        try {
          sessionStorage.setItem('consult_booking', JSON.stringify({
            ...
            /* The confirmation page cannot look this up: it is a different
               request with no funnel state. Without it a two-office client
               shows the wrong address on the page people screenshot. */
            office: v.office, office_label: v.office_label,
          }))
        } catch {}
      ...
      {gate === 'done' ? (
        <div className="done">
          ...
          <div className="receipt">
            ...
            {MULTI && <div><span>Office</span><b>{v.office_label}</b></div>}
        ...
        <>
          ...
          {STEPS[step].id === 'office' && (
            <div>
              <h2 className="step-title">Which office suits you?</h2>
              <p className="step-note">
                Both offer the same consultation. Pick the one you would travel to — a video
                consultation is offered at either, and you choose that on a later step.
              </p>
              <div className="formats">
                {OFFICES.map((o) => (
                  <button key={o.key} type="button" data-choice={o.key}
                          aria-pressed={v.office === o.key}
                          onClick={() => {
                            /* Changing office invalidates any slot already
                               picked: it belongs to the other practice's
                               calendar and would book into the wrong one. */
                            set('office', o.key)
                            set('office_label', o.label)
                            set('slot', '')
                            set('slot_label', '')
                          }}>
                    ...
                    <span className="ft">{o.label}</span>
                    <span className="fs">{o.address}, {o.city}</span>
                    <span className="fbadge">{o.phoneDisplay}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="formats">
                <button type="button" data-choice="in-person" aria-pressed={v.format === 'in-person'}
                  ...
                  <span className="fs">{whereLine}</span>
          ...
          {STEPS[step].id === 'time' && (
            <div>
              ...
              <p className="step-note">
                {CONSULT.durationLabel}, <b>{v.format === 'virtual' ? 'by video' : `in ${shortName}`}</b>.
                {' '}{CONSULT.hoursLine}
              </p>
              ...
              {slots && slots.length === 0 && (
                <p className="step-hint">No times are published for the next two weeks. Call {callNumber} and we will find one.</p>
              )}
              {slots && slots.length > 0 && (
                <div className="slots">
